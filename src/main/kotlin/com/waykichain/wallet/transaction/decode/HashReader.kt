package com.waykichain.wallet.transaction.decode
...
class HashReader(buf: ByteArray) : ByteArrayInputStream(buf) {
    ...
    fun readString():String{
        val len = this.readCompactSize()//VarInt(0).decode(this)
        val array = ByteArray(len.toInt())//len.value.toInt())
        this.read(array,0, len.toInt())//len.value.toInt())
        return String(array)
    }
...
//        val regId:WaykiRegId = WaykiRegId(height, index)
    ...
    fun readByteArray() :ByteArray{
        val len = this.readCompactSize()//readVarInt()
        val array = ByteArray(len.toInt())//.value.toInt())
        this.read(array,0, len.toInt())//.value.toInt())
        return array
    }
}